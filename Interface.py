...
# --- Définition des Constantes (Thème Sombre) ---
...
# Décommentez pour vérification en console au lancement
...
def open_dashboard(current_account: account):
    ...
    tk.Button(ops_frame, text="Déposer", command=do_deposit, 
              bg=colors["button_bg"], fg=colors["button_fg"], font=("Segoe UI", 10, "bold")
    ...
    def show_graph():
        ...
        # Style des textes (titre, labels)
        ax.set_title("Évolution du solde", fontsize=16, fontweight="bold", color=colors["text"])
        ax.set_xlabel("Heure de l'opération", fontsize=12, color=colors["text"])
        ax.set_ylabel("Solde (€)", fontsize=12, color=colors["text"])
        
        # Style des axes (graduations)
    ...
    tk.Button(dashboard, text=" Afficher l'évolution du solde", command=show_graph, 
              bg=colors["button_bg"], fg=colors["button_fg"], font=("Segoe UI", 12, "bold")
    ).pack(pady=30)
...
tk.Label(frame_login, text=" Connexion", font=("Segoe UI", 22, "bold"), 
         fg=colors["accent"], bg=colors["bg"]
).pack(pady=20)

# Champ Nom
tk.Label(frame_login, text="Nom :", font=("Segoe UI", 12), 
         fg=colors["text"], bg=colors["bg"]
).pack(pady=(10,0))

tk.Entry(frame_login, textvariable=username_var, font=("Segoe UI", 12), 
         bg=colors["entry_bg"], fg=colors["entry_fg"], insertbackground=colors["text"]
).pack(pady=5, padx=20)

# Champ ID
tk.Label(frame_login, text="ID du compte :", font=("Segoe UI", 12), 
         fg=colors["text"], bg=colors["bg"]
).pack(pady=(10,0))

tk.Entry(frame_login, textvariable=account_id_var, font=("Segoe UI", 12), 
         bg=colors["entry_bg"], fg=colors["entry_fg"], insertbackground=colors["text"]
).pack(pady=5, padx=20)

# Bouton de Connexion
tk.Button(frame_login, text="Se connecter", command=login, font=("Segoe UI", 12, "bold"), 
